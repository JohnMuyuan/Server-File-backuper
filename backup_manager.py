...
nav a{{color:var(--header-muted);padding:9px 12px;border-radius:10px;font-weight:650;transition:background .18s,color .18s,transform .18s}}nav a:hover{{background:var(--header-hover);color:var(--header-fg)}}nav a.active{{background:var(--header-active);color:var(--header-fg);box-shadow:inset 0 0 0 1px var(--header-border)}}nav a[href='/task']{{background:var(--primary);color:#fff}}nav a[href='/task']:hover{{background:var(--primary-hover);color:#fff;transform:translateY(-1px)}}
...
.running-list{{display:grid;gap:13px}}.running-card{{padding:16px;border:1px solid var(--border);border-radius:13px;background:var(--input);animation:card-in .25s both}}.running-top{{display:flex;gap:10px;align-items:center;margin-bottom:10px;min-width:0}}.running-meta{{display:flex;justify-content:space-between;gap:12px;margin:8px 0 0;color:var(--muted);font-size:13px}}.task-card{{display:flex;flex-direction:column;gap:12px;min-width:0;overflow:hidden}}.task-card>*{{min-width:0}}.task-card .card-heading>div{{min-width:0}}.task-card .card-heading h2,.running-top b{{overflow-wrap:anywhere;word-break:break-word}}.task-card .row:last-child{{margin-top:auto;padding-top:5px}}.empty-state{{text-align:center;padding:30px 18px;color:var(--muted)}}
