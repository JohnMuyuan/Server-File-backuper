...
3. 选择“文件 / 目录”或数据库类型。文件任务可选择“快照压缩包”“增量归档（只新增、不压缩）”或“完全镜像同步（不压缩）”，再设置“每隔 N 天”和一个或多个备份时间。例如每隔 1 天，在 02:00、14:00 运行，就是每天两次。
...
网页首页显示备份磁盘占用圆环、各任务占用、服务器实时上传/下载网速、紧凑且可展开的后续备份队列、运行阶段和最近日志；日志页和任务详情页按需加载最近日志，可继续加载更早记录或下载完整应用日志，避免大量日志一次性渲染导致页面卡顿。页面已适配手机宽度和触控操作。每张任务卡都有暂停、停止清除、继续和“详情”操作；备份、暂停和停止会显示即时加载反馈，成功后自动返回首页。详情页可查看实际并发策略、活跃传输槽位的文件/速度/已传输量、已有备份文件和该任务的分页日志。失败原因会优先显示捕获到的认证、权限、网络或文件错误，不再用末尾的正常传输记录代替。
