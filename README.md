...
网页首页显示备份磁盘占用圆环、各任务占用、服务器实时上传/下载网速、后续多次备份队列、运行阶段和最近日志；日志页显示当前及上一轮转的全部应用日志。每张任务卡都有“详情”，可查看总体进度、活跃传输槽位的文件/速度/已传输量，以及该任务的完整日志。主题图标会在日间、夜间和跟随系统之间切换，并提供即时动画反馈。
