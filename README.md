...
3. 填写源服务器和路径，再设置“每隔 N 天”和一个或多个备份时间。例如每隔 1 天，在 02:00、14:00 运行，就是每天两次。
...
默认 4 线程，允许范围为 1-16：多个文件可并行下载，大文件可分段下载；中断内容保存在每个任务自己的 `.partial-任务ID` 目录中，下次自动续传。线程数设为 1 时使用 rsync 兼容模式。

程序先把远程文件镜像到备份服务器的暂存目录，再在备份服务器本机生成 `.tar.zst` 压缩包，成功后删除暂存目录。源服务器不会产生压缩文件；下载失败可断点续传，压缩失败也会保留已下载内容。该压缩包是一次完整备份产物，但不是原子文件系统快照；数据库等持续写入的数据仍应先导出或创建文件系统快照。
...
网页首页显示备份磁盘占用圆环、各任务占用、服务器实时总网速、后续多次备份队列、运行阶段和重要日志；日志页可以查看近期日志并下载当前及上一轮转的完整应用日志。主题图标可在日间、夜间和跟随系统之间切换。
